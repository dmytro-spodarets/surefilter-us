...
- [x] **Security audit pack (May 2026 best practices)** (2026-05-17, commit `1de974f`): full sweep of every admin / public / MCP API against MCP Spec 2025-11-25 + OWASP API Security Top 10 (2023, current for 2026) + post-CVE-2025-29927.
  - **P0** — PII redaction in form + banner submissions (email / IP / userAgent / data-JSON masked for non-`admin:*`); `/api/admin/cache` now goes through `requireAdmin()` (previously a session-only check with no role gate).
  - **P1** — tools/list filter (anon sees 14 public read tools instead of 80; reader depends on scopes; `admin:*` sees all 81); race-safe idempotency (atomic INSERT-or-fail with PENDING_SENTINEL → outcomes claimed/cached/in-progress); 9 admin routes converted from ad-hoc `getServerSession + role` to canonical `requireAdmin()`; users/[id] self-check bug fixed.
  - **P2** — folder ops now write AdminLog entries (entityType='S3Folder'); audit.ts secret-key sanitizer switched to substring match.
  - Smoke 27/27. Image `v1.3.0` should be rebuilt as `v1.3.1` to ship these fixes.
...
- [ ] **MCP follow-up: wide `withIdempotency` rollout** — all 50 write tools currently accept `idempotencyKey` in their Zod input, but the claim → run → finalize wiring is live only in `content-create-news-category` and `content-create-resource-category` (kept as reference impls). Roll the wrapper out across the rest gradually as MCP usage grows; do it one tool at a time via the Edit tool — a bulk perl regex broke 7 files last time. Groups still to cover: catalog-writes (3), content-writes (10), cms-writes (6), forms-writes (3), banners-writes (5), media-writes (6), users-writes (3), admin-writes (1).
