...
- 2026-04-27 — Popup Banner System (Phase 1): собственная система попап-банеров вместо HelloBar — Prisma модели (Banner/BannerCampaign/BannerImpression/BannerClick/BannerSubmission + 4 enum), 1-минутный server-side cache в `lib/banners.ts` с `clearBannersCache()`, public API `/api/banners/active|impression|click` (sendBeacon-friendly, rate-limited, full DB logging), admin API CRUD `/api/admin/banners`, layout registry с 3 стартовыми вариантами (ClassicCentered/SideImage/MinimalText) + SVG-превью, BannerHost orchestrator с targeting+UTM+referer+dismiss+delay (3 режима: SESSION/DAYS/FOREVER), native `<dialog>` modal, admin UI с tabs (Layout gallery/Content/Targeting/Triggers/Schedule/Lead) + sticky live-preview, Marketing nav menu
- 2026-04-20 — Newsroom redesign: новый CMS-компонент `ColorHero` (solid background color вместо картинки, уменьшенная высота), newsroom hero переведён на ColorHero с настраиваемым цветом из SiteSettings (newsroomHeroColor), hero убран со страницы отдельной статьи, featured image перемещён наверх карточки статьи, надпись "Latest News" показывается только при наличии featured events
- 2026-04-20 — Next.js config fix: удалён устаревший `experimental.nodeMiddleware` (в Next.js 15.5.12 достаточно `export const runtime = 'nodejs'` в middleware.ts)
- 2026-04-14 — SEO duplicate content fix (ahrefs 186 дублей): (1) CloudFront Function `set_x_forwarded_host` расширена — теперь делает edge-редирект www.surefilter.us / new.surefilter.us → surefilter.us (301) на viewer-request до lookup в кэше, решает проблему cache-key sharing между алиасами; (2) добавлены `<link rel="canonical">` через `alternates.canonical` в generateMetadata() всех публичных страниц (/, catch-all [...slug], /newsroom, /newsroom/[slug], /resources, /resources/[category]/[slug], /products/[code]); (3) удалён мёртвый non-canonical host redirect из middleware.ts (не работал — CF отдавал App Runner host в Host header)
...
- 2026-03-07 — Performance & Security Audit (round 2): 14 дополнительных исправлений — SEC-11 folder rename fix, P4 lazy loading CMS секций (next/dynamic), P6 skeleton loading.tsx (catalog/newsroom/resources), P7 client→server components (4 hero), P8 warm-up расширен до 30 путей (из sitemap), P11 мемоизация shimmer, P12 fix CLS /resources (Suspense fallback), SEC-14 HSTS preload, SEC-16 rate limiting public endpoints, SEC-17 pagination bounds, SEC-18 CSS whitelist sanitize.ts, SEC-19 JWT maxAge 24h
- 2026-03-06 — Security Audit: комплексный аудит безопасности перед продакшн-релизом — 25 исправлений: auth checks на 20+ admin API routes (requireAdmin helper), SSRF prevention (url-validator.ts), path traversal fix в folder operations, HTML sanitization (sanitize-html), rate limiting (формы, каталог пароль, публичные API), ReDoS protection (safe-regex2), timing-safe password comparison, pagination limits, Zod validation, security headers (CSP, HSTS, X-Frame-Options, Permissions-Policy), server-only imports, error message leaking fixes, webhook timeout, TinyMCE API key перенесён из публичного endpoint в admin-only `/api/admin/config/tinymce` (runtime из SSM через App Runner, не build-time), NEXT_SERVER_ACTIONS_ENCRYPTION_KEY в инфраструктуре
...
- 2026-03-04 — Termly CMP: интеграция cookie consent баннера — поле Website UUID в SiteSettings (БД + админка Settings → Security), клиентский компонент TermlyCMP с autoBlock, ссылка Consent Preferences в футере
...
- 2026-03-02 — robots.txt: все AI-боты разрешены (training + retrieval), стратегия — максимальная видимость в AI-поисковиках
- 2026-03-02 — SEO/GEO аудит: полный аудит кода на соответствие best practices 2026, 23 задачи добавлены в TODO.md
...
- 2025-12-29 — Все изображения перенесены в S3: isAssetPath теперь поддерживает пути с / и без (/images/file.jpg и images/file.jpg), все конвертируются в CDN URL
- 2025-12-29 — КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: isAssetPath теперь правильно различает локальные файлы (/images/) от S3 (industries/), исправлены 403 ошибки для логотипа и статичных изображений
- 2025-12-29 — Убран priority из всех hero компонентов: исправлены предупреждения preload, priority только для критичных LCP изображений
