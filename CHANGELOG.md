...
### История
- 2026-03-06 — Security Audit: комплексный аудит безопасности перед продакшн-релизом — 25 исправлений: auth checks на 20+ admin API routes (requireAdmin helper), SSRF prevention (url-validator.ts), path traversal fix в folder operations, HTML sanitization (sanitize-html), rate limiting (формы, каталог пароль, публичные API), ReDoS protection (safe-regex2), timing-safe password comparison, pagination limits, Zod validation, security headers (CSP, HSTS, X-Frame-Options, Permissions-Policy), server-only imports, error message leaking fixes, webhook timeout, TinyMCE API key endpoint removal, NEXT_SERVER_ACTIONS_ENCRYPTION_KEY в инфраструктуре
