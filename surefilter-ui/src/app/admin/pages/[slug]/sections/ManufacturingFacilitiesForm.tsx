...
import type { ManufacturingFacilitiesInput } from '@/cms/schemas';

export default function ManufacturingFacilitiesForm({ sectionId, initialData }: { sectionId: string; initialData: ManufacturingFacilitiesInput }) {
  const [form, setForm] = useState<ManufacturingFacilitiesInput>({
    title: initialData.title || 'Manufacturing Facilities',
  ...
  const updateItem = (idx: number, key: keyof ManufacturingFacilitiesInput['items'][number], value: string) => {
    setForm((f) => {
      const items = [...f.items];
  ...
  const addItem = () => setForm((f) => ({ ...f, items: [...f.items, { title: '', description: '', icon: 'BuildingOffice2Icon', image: '' }] }));
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    await fetch(`/api/admin/sections/${sectionId}`, {
      ...
      body: JSON.stringify({ type: 'manufacturing_facilities', data: form }),
    });
    setSaving(false);
    setSaved(true);
  };
  ...
  return (
    <section className="border border-gray-200 rounded-lg p-5">
      <h2 className="text-lg font-medium text-gray-900 mb-4">Manufacturing Facilities</h2>
