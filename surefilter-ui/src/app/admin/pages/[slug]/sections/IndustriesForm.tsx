...
import type { IndustriesInput } from '@/cms/schemas';

export default function IndustriesForm({ sectionId, initialData }: { sectionId: string; initialData: IndustriesInput }) {
  const [form, setForm] = useState<IndustriesInput>({
    title: initialData.title || 'Industries We Serve',
    description: initialData.description || 'Comprehensive filtration solutions for every heavy-duty application',
    items: Array.isArray(initialData.items) ? initialData.items : [],
  });
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  const updateItem = (idx: number, key: keyof IndustriesInput['items'][number], value: string) => {
    setForm((f) => {
      const items = [...f.items];
  ...
  const addItem = () => setForm((f) => ({ ...f, items: [...f.items, { name: '', description: '', image: '', href: '' }] }));
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    await fetch(`/api/admin/sections/${sectionId}`, {
      ...
      body: JSON.stringify({ type: 'industries', data: form }),
    });
    setSaving(false);
    setSaved(true);
  };
  ...
  return (
    <section className="border border-gray-200 rounded-lg p-5">
      <h2 className="text-lg font-medium text-gray-900 mb-4">Industries</h2>
