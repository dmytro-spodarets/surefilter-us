...
export default function CompactSearchHeroForm({ sectionId, initialData }: { sectionId: string; initialData: any }) {
  ...
  const onSave = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(`/api/admin/sections/${sectionId}`, {
        ...
        body: JSON.stringify({ type: 'compact_search_hero', data: { title, description, image } }),
      });
    } finally {
      setSaving(false);
    }
