...
import type { PopularFiltersCatalogInput } from '@/cms/schemas';
...
export default function PopularFiltersCatalogForm({ 
  sectionId, 
  initialData 
}: { 
  sectionId: string; 
  initialData: PopularFiltersCatalogInput 
}) {
  const [form, setForm] = useState({
    title: initialData?.title || 'Popular Filters',
    description: initialData.description || 'Top-selling filters for heavy duty applications',
    catalogHref: initialData.catalogHref || '/catalog',
    catalogText: initialData.catalogText || 'Browse All Filters',
    columnsPerRow: initialData.columnsPerRow || 5,
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    await fetch(`/api/admin/sections/${sectionId}`, {
      ...
      body: JSON.stringify({ type: 'popular_filters_catalog', data: form }),
    });
    setSaving(false);
    setSaved(true);
  };
  ...
  return (
    <section className="border border-gray-200 rounded-lg p-5">
      <h2 className="text-lg font-medium text-gray-900 mb-4">Popular Filters (Catalog)</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input 
              ...
              onChange={(e) => setForm({ ...form, catalogText: e.target.value })} 
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input 
              ...
              onChange={(e) => setForm({ ...form, catalogHref: e.target.value })} 
            />
          </div>
          <div>
            ...
            <input 
              ...
              max={6}
              className="w-full border border-gray-300 rounded-lg px-3 py-2" 
              value={form.columnsPerRow || 5} 
              onChange={(e) => setForm({ ...form, columnsPerRow: parseInt(e.target.value) || 5 })} 
            />
          </div>
        </div>
