...
import type { FeaturedProductsInput } from '@/cms/schemas';

export default function FeaturedProductsForm({ sectionId, initialData }: { sectionId: string; initialData: FeaturedProductsInput }) {
  const [form, setForm] = useState<FeaturedProductsInput>({
  ...
  const updateItem = (idx: number, key: keyof FeaturedProductsInput['items'][number], value: string) => {
    setForm((f) => {
      const items = [...f.items];
  ...
  const addItem = () => setForm((f) => ({ ...f, items: [...f.items, { name: '', description: '', image: '', category: '', href: '' }] }));
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    await fetch(`/api/admin/sections/${sectionId}`, {
      ...
      body: JSON.stringify({ type: 'featured_products', data: form }),
    });
    setSaving(false);
    setSaved(true);
  };
      ...
      <form onSubmit={onSubmit} className="grid gap-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="w-full border border-gray-300 rounded-lg px-3 py-2" value={form.fallbackHref || ''} onChange={(e) => setForm({ ...form, fallbackHref: e.target.value })} />
          </div>
        </div>
        <div>
            ...
            <div key={idx} className="border border-gray-200 rounded-lg p-4 grid gap-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input className="w-full border border-gray-300 rounded-lg px-3 py-2" value={it.name} onChange={(e) => updateItem(idx, 'name', e.target.value)} />
                </div>
                <div>
                  <label className="block text-sm text-gray-700 mb-1">Category</label>
                  <input className="w-full border border-gray-300 rounded-lg px-3 py-2" value={it.category || ''} onChange={(e) => updateItem(idx, 'category', e.target.value)} />
                </div>
              </div>
              <div>
                <label className="block text-sm text-gray-700 mb-1">Description</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-sm text-gray-700 mb-1">Link (href)</label>
