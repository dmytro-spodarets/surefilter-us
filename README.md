...
- Домен и CDN: `surefilter.us` через CloudFront + ACM + Route53. Канонический домен принудительно в `middleware`, origin защищён заголовком `X-Origin-Secret`. Редирект с `www` и `new` → `surefilter.us` (301). Старый сайт доступен по `old.surefilter.us`.
...
**Проблема**: Сайт работал по прямому App Runner URL (`https://qiypwsyuxm.us-east-1.awsapprunner.com/`) но возвращал 404 через CloudFront (`https://surefilter.us/`).
...
1. **ci-build-push.yml** - Сборка и публикация Docker образа
   ...
   - Публикует в AWS ECR
   - Требует ручного обновления `infra/envs/prod/image-versions.tf` (переменная `app_runner_image_version`)
...
3. **Обновить версию в Terraform**
   ```bash
   # Редактировать infra/envs/prod/image-versions.tf
...
4. **Применить Terraform**
   ...
   # или разово, без правки файла:
   # tofu apply -var-file=secrets.tfvars -var="app_runner_image_version=vX.Y.Z"
   ```
